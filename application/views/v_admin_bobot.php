        ...
        Bobot
					...
					for($awal = $minimal_kriteria; $awal<=$maksimal_kriteria; $awal++){
						for($akhir = $minimal_kriteria; $akhir <=$maksimal_kriteria; $akhir++){
							$nilai_perbandingan_kriteria[$awal][$akhir] = $nilai_awal_kriteria[$akhir]/$nilai_awal_kriteria[$awal];
						}
					}
					...
					for($awal = $minimal_kriteria; $awal<=$maksimal_kriteria; $awal++){
						for($akhir = $minimal_kriteria; $akhir <=$maksimal_kriteria; $akhir++){
							$nilai_prioritas_kriteria[$awal][$akhir] = $nilai_awal_kriteria[$akhir]/$nilai_awal_kriteria[$awal]/$jumlah_nilai_perbandingan_kriteria[$akhir];
						}
					...
					for($kriteria_id=$minimal_kriteria; $kriteria_id<=$maksimal_kriteria; $kriteria_id++){
						...
						for($x = $minimal_alternatif[$kriteria_id]; $x<=$maksimal_alternatif[$kriteria_id]; $x++){
							...
							foreach ($query->result_array() as $row){
								...
								$id_awal_alternatif[$x] = $row['siswa_id'];
							}
						}
				...
				<h5><strong>Matriks Hasil (Bobot)</strong></h5>
				  <table id="example1" class="table table-bordered table-striped">
					...
					<tr>
						<th>Alternatif</th>
						<?php for($w = $minimal_kriteria; $w<=$maksimal_kriteria; $w++){
								echo "<td style=\"text-align:center;\">".round($bobot_prioritas_kriteria[$w],2)."</td>";
							}
						?>
					</tr>					
					</thead>
					...
					<tr>
						...
						<?php for($x = $minimal_kriteria; $x<=$maksimal_kriteria; $x++){
								$m = $w+($count_alternatif*($x-1));?>
						<td> <?php echo round($bobot_prioritas_alternatif[$m][$x],2);?> </td>
						<?php } ?>
						...
						<?php 
							echo round($prioritas_pilihan_alternatif[$w],2);
						?>
						</td>
